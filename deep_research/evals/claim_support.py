...
async def async_eval_claim_support(report_markdown: str, writer_evidence: list[dict]) -> tuple[float, str]:
    """Async: check whether major claims are supported by cited evidence (LLM-as-judge)."""
    if not report_markdown:
        return 0.0, "Empty report"
    if not writer_evidence:
        return 0.0, "No evidence provided to writer"
