...
6. Runs an **eval stop gate** (LLM-as-judge on `research_trace` + gaps) that can trigger **one** extra pass through conflict resolution if research looks insufficient
7. Prepares a curated evidence subset for the writer
8. Writes section drafts, assembles the report, and appends it to the conversation
...
│       → [conflict_resolution?] → eval_stop_gate → prepare_writer_context          │
│       → write_sections → write_report → finalize → END                             │
│  (eval_stop_gate may route back to conflict_resolution_research once; see §5.10–11) │
└─────────────────────────────────────────────────────────────────────────────────┘
...
- **detect_global_gaps_and_conflicts** identifies conflicts in merged evidence; **conflict_route** sends the graph either to **conflict_resolution_research** (targeted search + LLM adjudication) or to **eval_stop_gate** (skipping resolution when not needed or when disabled). **Both** paths always pass through **eval_stop_gate** before **prepare_writer_context**. **stop_eval_route** can send the run back to **conflict_resolution_research** at most once if the stop eval judges research insufficient. Resolution is optional (config: `conflict_resolution_enabled`).
...
| `error_message` | Search / config failure; routing stops the graph | `_keep_first` (parallel workers) |
...
| `conflict_resolution_enabled` | Mirrored from config in detect node; used by **conflict_route** | — |
| `global_seen_urls` | Union of URLs across section merge / conflict resolution | `_merge_sets` |
| `section_max_iterations` | Per-section iteration cap from decompose | `_keep_first` |
| `stop_eval_score` | 0–1 score from **eval_stop_gate** (`async_eval_stop_decision`) | — |
| `research_sufficient` | Whether to proceed to writer after stop eval | — |
| `research_retry_count` | Increments each **eval_stop_gate** visit; bounds retry loop | — |
...
| `error_message` | Search API failure; subgraph ends early; parent may stop run | — |
...
- **JSON fallbacks:** Malformed JSON triggers a **minimal default plan** (see **§5.4 — JSON parsing** subsection below, shared with decompose).
...
#### JSON parsing (`create_research_plan` + `decompose_into_sections`): fallbacks, silent degradation, hardening

Both nodes use the **custom parser** pattern (§27): strip markdown **code fences** (e.g. a leading `json` language tag or a plain fenced block), then **`json.loads`**. On **`JSONDecodeError`**, the graph **does not stop**—it substitutes **defaults** and continues.

| Node | On `JSONDecodeError` |
|------|----------------------|
| **`create_research_plan`** | Replaces with a **minimal plan**: objective = user query, **`desired_structure`** = one “Overview” section, empty difficulty metadata (`planner.py`). |
| **`decompose_into_sections`** | Uses **`{"section_tasks": []}`**, which triggers the **single fallback section** (“Overview”, generic goal / questions) (`decompose.py`). |
| **Phase 1 `plan_and_generate_queries`** | **`search_queries: []`**, **`report_outline`** kept from previous iteration (`planner.py`). |

**Risks (silent degradation)**

- **Wrong user-visible outcome:** HITL may **approve** a plan that is mostly **fallback**, not the model’s real output.
- **Lost structure:** A rich multi-section decomposition collapses to **one** generic section → workers and merge still run, but research is **mis-scoped**.
- **Phase 1 empty search:** **`search_queries: []`** can yield **weak or empty** search while the run **completes**.
- **“Valid JSON, bad shape”:** If **`json.loads` succeeds** but fields are **wrong types** (e.g. `desired_structure` is a string), downstream **`.get` / iteration** may behave oddly **without** hitting the `except` path.
- **Fence extraction:** Taking the **first** fenced block can pull **wrong** content if the model emits multiple blocks or prose-wrapped JSON.
- **Ops noise:** **`research_trace`** can still look “normal” (e.g. `sections_created: 1`) with **no** explicit **`json_parse_ok: false`** flag today.

**How to harden**

1. **Telemetry first:** On parse or schema failure, set **`research_trace["plan_json_parse_ok"]` / `decompose_json_parse_ok`** (or similar), **WARN** log with a **short** raw-text preview, and surface in UI (“Plan used JSON fallback”).
2. **Structured output:** Prefer **`with_structured_output`** / tool calling / provider **JSON mode** for plan + decompose so the runtime **validates** shape before state updates.
3. **Post-parse validation:** **Pydantic** models for required keys and list/dict types; treat validation failure like **`JSONDecodeError`** (retry or fail loudly).
4. **One bounded retry:** Second LLM call: “Return **only** valid JSON matching this schema: …”
5. **HITL:** On failure, **interrupt** or require **regenerate** instead of silently substituting a generic plan when plan approval is enabled.
6. **Phase 1:** If **`search_queries`** is empty after planner, **short-circuit** or **retry** rather than calling search with no queries.

**Interview shorthand:** Fallbacks favor **availability over integrity**—the run succeeds but **intent** can be **replaced** without a clear signal. **Fix** with **flags + logs + structured output + schema validation + retry or HITL**.
...
- **Rate limits:** Large `max_parallel_sections` stacks with per-worker **`asyncio.gather`** in `section_search` and with **`section_max_iterations`**, so outbound search and LLM traffic scales multiplicatively. Mitigations (semaphores at `_run_one_query_async`, per-provider RPM, separate LLM caps, tuning caps) are spelled out in [§25 — Rate-limit strategy](#25-error-handling-retries-fallbacks-and-rate-limit-strategy).
...
- **Logic:** If `conflict_resolution_needed` and `conflict_resolution_enabled` → go to **conflict_resolution_research**; else → **eval_stop_gate** (the graph maps this branch to the `eval_stop_gate` node; it is **not** `prepare_writer_context` directly).
- **Why:** Conflict resolution runs only when needed and enabled; **every** path then goes through **eval_stop_gate** before writer prep so stop-quality checks apply uniformly.

---

### 5.10 eval_stop_gate

**File:** `deep_research/nodes/conflicts.py` — `eval_stop_gate()`

- **Input:** `research_trace`, `knowledge_gaps` (Phase 1 field; often empty in Phase 2), `research_retry_count`.
- **Behavior:** Calls **`async_eval_stop_decision`** (`deep_research/evals/stop_decision.py`) — an LLM-as-judge rubric on whether the agent stopped at the right time given trace + gaps. Produces a **0–1** `stop_eval_score`. Sets **`research_sufficient`** to true if `score >= 0.6` **or** `research_retry_count >= 1` (so a second visit always proceeds). Increments **`research_retry_count`** each time the node runs.
- **Output:** `stop_eval_score`, `research_sufficient`, `research_retry_count`.
- **Why:** Optional quality gate before expensive writing; **stop_eval_route** (§5.11) can loop back to **conflict_resolution_research** once when the eval flags insufficient research.

---

### 5.11 stop_eval_route (conditional edges)

**File:** `deep_research/routing.py` — `stop_eval_route()`

- **Logic:** If **not** `research_sufficient` **and** `research_retry_count <= 1` → **conflict_resolution_research** (retry targeted resolution); else → **prepare_writer_context**.
- **Why:** Bounded retry (at most one extra conflict-resolution pass after the first eval) before committing to the writer.

---

### 5.12 conflict_resolution_research (optional)
...
- **Behavior:** Converts merged evidence to writer format (section_ids from supporting_sections). Scores by relevance, primary, redundancy. **First** tries to attach **at least one distinct URL per outline section** (reservation pass). **Then** adds cross-cutting sources and fills by relevance, but only while **`len(chosen) < writer_context_max_items`**—so the hard cap applies to **cross-cutting + fill**, not to the initial per-section reservation (see below). Optionally enriches with full-page content (Tavily Extract, Exa get_contents, or trafilatura).
- **Output:** `writer_evidence_subset`, `research_trace`.
- **Why:** Writer gets a section-aware, high-quality set of sources; capping the later phases limits token growth while preserving breadth when the outline is small.
- **When `writer_evidence_subset` is empty:** If both **`merged_evidence`** and **`evidence_items`** are empty, the node returns `{"writer_evidence_subset": []}` immediately (no research to curate). If evidence exists but **every item lacks a usable URL**, selection loops never append to `chosen`, so the subset can also end up empty despite non-empty upstream lists (bad or incomplete normalization).

#### `writer_context_max_items`: how the default was chosen and how to tune it

**Defaults in repo:** `deep_research/configuration.py` sets **`DEFAULT_WRITER_CONTEXT_MAX_ITEMS = 30`**. Sample **`config.yaml`** uses **`section.context_max_items: 50`** (mapped to `writer_context_max_items`). There is **no** automated search over this hyperparameter in code—it is a **heuristic**:

- **Context budget:** Writer prompts JSON-encode **url / title / snippet** (and assembly mode still lists **sources**). The cap keeps **total characters/tokens** roughly in line with a **strong model’s** comfortable context after outline, summaries, and instructions—especially when **`fetch_full_pages`** inflates snippets via **`full_page_max_chars`**.
- **Quality vs. noise:** More items add diversity but dilute focus and increase **contradiction / lost-in-the-middle** risk; **30–50** is a practical “enough sources, not a dump of everything merged” range for **moderate section counts**.
- **Cost:** Every extra item increases prompt size for paths that pass the full subset into **one** writer call (enhanced/basic Phase 2); Phase 2 with **`write_sections`** still uses the **global** subset for citation indexing and assembly sources, so a very large subset matters for **assembly** and for **per-section** bucketing volume indirectly.

**Tuning for a 3-section brief**

- **Lower** the cap (e.g. **15–25**) when the outline is short: you need fewer “at least one per section” slots, and shorter prompts reduce latency and cost.
- If **`fetch_full_pages`** is **on**, bias **lower** still (each snippet can be **thousands** of characters)—tune together with **`full_page_max_chars`**.
- **Phase 2 + `write_sections`:** Each section prompt only sees **evidence bucketed to that section**, but the **curator still builds one global list**; a small report rarely needs dozens of global sources.

**Tuning for a ~50-section report**

- **Raise** the cap (e.g. **80–150**) if you rely on the **global** enhanced/basic writer or on a **large sources block** in assembly—otherwise the **fill** phase may add almost nothing once the reservation pass has consumed budget.
- **Important implementation detail:** The **per-section reservation loop** does **not** check **`writer_context_max_items`**. With **many outline sections**, `chosen` can already be **large** (up to one URL per section that finds a match) **before** cross-cutting and fill run; those steps only append while **`len(chosen) < max_items`**. So for **very large outlines**, **`writer_context_max_items` does not strictly bound** total items today—production hardening might enforce the cap on **all** phases or cap **outline sections** passed to the writer path.
- **Prefer section-scoped writing for huge reports:** Phase 2 **`write_sections`** limits **per-call** context to **one section’s** evidence bucket; combine with **reasonable `max_parallel_sections`** and **merge** quality so each section still gets strong sources without one prompt holding **50 sections × N snippets**.

**Interview shorthand:** The number is a **hand-tuned default** for **token/cost/quality** tradeoffs, not learned. **Brief → lower cap + shorter snippets; mega-outline → raise cap and/or fix curation so `max_items` truly bounds list size, and rely on per-section writing to scale.**
...
#### Empty `writer_evidence_subset` → fallback to `merged_evidence`

In `write_report`, the primary evidence list is `writer_evidence_subset`. If it is **empty**, the node **falls back** to **`merged_evidence`**, mapping each merged item to `{url, title, snippet}` for the prompt and `sources` (`deep_research/nodes/writer.py`). The inline comment cites **checkpoint / state-merge quirks**: e.g. interrupt/resume or partial persistence where the field written in `prepare_writer_context` is missing or empty while **`merged_evidence`** from `merge_section_evidence` is still present—this fallback avoids calling the writer with no sources when merge data actually exists.

**Why the empty case happens (summary):**

| Situation | Effect |
|-----------|--------|
| No `merged_evidence` and no Phase 1 `evidence_items` | `prepare_writer_context` returns `[]`; fallback sees empty `merged_evidence` too → writer still has **no evidence**. |
| Evidence without URLs | Curator may produce `[]`; merge may also have dropped URL-less rows. |
| State / checkpointer edge cases | `writer_evidence_subset` absent or cleared while `merged_evidence` remains → fallback **recovers** grounding from merge. |

**Is the fallback sufficient for production?**

- **Good as a safety net** when the only failure mode is “subset lost, merge intact”: the writer still receives url/title/snippet consistent with what enhanced/basic modes JSON-encode.
- **Not sufficient alone:** If **`merged_evidence` is also empty**, the report can be **ungrounded**; there is no hard stop or user-visible warning in `write_report`.
- **Silent degradation:** Full-page enrichment (`fetch_full_pages`) updates snippets in the **curated** list; **`merged_evidence` keeps original snippets**. If the subset was lost but merge survived, the fallback may use **shorter** context than intended without any signal.
- **Production hardening:** Log or metric when fallback triggers; treat **zero evidence** after fallback as an error or explicit “insufficient research” path; verify checkpointed threads persist both merge and subset as expected.

#### Report cites a URL that never appeared in evidence — where that can leak

Clarify what “never appeared in evidence” means:

- **Markdown body** (narrative or a model-written “Sources” block) contains a **URL or citation** that was **not** in the curated list passed to the writer.
- Versus **`state["sources"]`** after `write_report`: that list is **not** parsed from the model output.

**How `sources` is built:** `write_report` enumerates the same in-memory **`evidence`** list it used for the prompt (after optional **`merged_evidence`** fallback) and appends `{index, url, title}` — **deterministic**, not extracted from `report_markdown`:

```153:159:deep_research/nodes/writer.py
    sources = []
...
So **bogus URLs in the prose** while **`sources` is clean** is a **generation** issue, not a bug in `sources` assembly.

| Where it leaks | Mechanism |
|----------------|-----------|
| **`write_report`** | Final markdown is **free-form**. **Assembly** mode sees stitched **`section_drafts`** plus a string **`sources_list`**; the model can still **invent URLs**, add an extra “Sources” section, or mis-cite. **Enhanced/basic** modes pass JSON evidence but the output is still unconstrained text. |
| **`write_sections`** | Each **section draft** is free text. The prompt includes **`citation_index`**, **url**, **snippet** per item, but the model can still **paste arbitrary URLs** or misuse **`[n]`** in prose; **`write_report`** may further rewrite when assembling. |
| **Upstream (less common)** | **`normalize` / `section_normalize`**: structured LLM output could emit a **`url`** not present in raw search rows if you never **validate against the raw URL set**. **`merge`** / **`prepare_writer_context`** are largely deterministic on those rows—failures are usually drops/duplicates, not random new domains. |
| **Definition mismatch** | A URL exists in **`merged_evidence`** but was **dropped** from **`writer_evidence_subset`**. The writer should not “know” it, yet the model may **hallucinate** a plausible link—that is still a **writer** failure, not proof the URL was in the writer prompt. |

**How to prevent**

1. **Post-validate (deterministic):** Extract URLs from **`report_markdown`**; allow only URLs in the **allowed set** (e.g. curated evidence / merge) or only numeric **`[n]`** with **n ≤ len(sources)**. On violation: strip, **repair** prompt, or **fail** the run.
2. **Machine-built Sources block:** Append or replace the report’s sources section from the same list used for **`sources`**; forbid or strip a duplicate model-generated list.
3. **Stricter citation contract:** Prompts that require **`[n]` only** (no bare URLs in body), or a second pass that rewrites citations to valid indices.
4. **Normalize hardening:** After structured normalize, **drop** items whose **`url`** was not in the **raw** results for that step.
5. **Evals / gates:** Use **`citation_relevance`** and related evals (§22) as quality signals; add a **deploy gate** or **citation_integrity** metric (see §5.17).

**Interview shorthand:** **`sources` in state mirrors the evidence list, not the LLM’s report.** Spurious URLs almost always come from **`write_sections` / `write_report`** **hallucination**. **Fix** with **allowlists, post-checks, non-LLM Sources sections**, and **validated normalize output**.

---

### 5.16 finalize_messages
...
### 5.17 Debugging: wrong final report when `merged_evidence` and `prepare_writer_context` look correct

If **`merged_evidence`** and the curated **`writer_evidence_subset`** look fine in state but the **final report is wrong**, the failure is usually **downstream of curation**: what **`write_sections`** / **`write_report`** actually received, how **assembly** rewrote **`section_drafts`**, or **model behavior** (ignored instructions, truncation). Treat “state looks correct” as a hypothesis until you compare it to **node inputs at invoke time** (checkpoint/resume and UI snapshots can disagree with the payload passed to the LLM).

#### Debugging order

1. **Inspect writer node I/O in LangSmith / Studio (or replay from checkpoint)** — Confirm **`write_sections`** and **`write_report`** inputs match the evidence you reviewed.
2. **Phase 2 with `section_drafts`:** Compare each **section draft** to the **assembled `report_markdown`**. Assembly-only bugs (tone, omissions, smoothing over contradictions) are common even when drafts and merge are good.
3. **`global_conflicts`:** **`write_report`** injects formatted conflict context into the prompt (`_format_conflict_resolutions`). Noisy or wrong conflict text can steer the model despite correct evidence lists.
4. **Citations vs `sources`:** **`write_report`** builds **`sources`** from the **`evidence`** list it used (after optional **`merged_evidence`** fallback). Compare bracketed **`[n]`** citations in the markdown to **`sources`** and to curated URLs; mismatches indicate **hallucinated citations** or assembly drift, not merge/prepare.
5. **Replay:** Re-invoke **`write_report`** alone with frozen **`section_drafts`**, **`writer_evidence_subset`**, **`report_outline`**, **`global_conflicts`**. If the bad report reproduces at temperature 0, the issue is **prompt + model**; if not, look at **nondeterminism** or **truncation**.

#### What we log today (`deep_research/research_logger.py`, `run.py`)

| Mechanism | Scope |
|-----------|--------|
| **`--log` / `--log-file`** | **Off by default.** When enabled: **node start/end**, **routes**, **decisions**; **`log_prompt`** with **user prompt truncated to ~4000 chars** (system ~800); output summaries **~2000 chars**. **No PII redaction** (see appendix on logging). |
| **`prepare_writer_context`** | **`log_node_start` / `log_node_end`** with **`writer_evidence_count`**, **`sections_covered`**; optional **full-page cache** logging / `log_cache_event`. Does **not** dump full **`writer_evidence_subset`** by default. |
| **`write_sections`** | **`log_prompt`** per section as **`write_section_{sid}`** (truncated); **`log_node_end`** with **`sections_written`**, **`total_evidence`**. **Draft text** is not logged in **`log_node_end`**. |
| **`write_report`** | **`log_prompt`** (truncated); **`log_node_end`** with **`report_length`**, **`evidence_count`**. |
| **`--trace`** | Writes **`research_trace`** JSON (counts, **`writer_evidence_count`**, conflicts, etc.). **Not** a full evidence payload; **`run.py`** may **redact** large fields for LangSmith size limits. |
| **LangSmith** | **`--langsmith`**: traces with timing and (depending on flags) **inputs/outputs** — best place to see **exact** writer prompts if not redacted. |

**Gaps today:** No log field for **`used_merged_evidence_fallback`** in **`write_report`**, no explicit **`writer_mode`** (`assembly` vs `enhanced` vs `basic`) in trace, no **fingerprint** of the evidence list passed into the final prompt.

#### What to add (production-style)

1. **`research_trace` (or structured logs):** **`writer_mode`**, **`used_merged_evidence_fallback`**, **`writer_evidence_subset_len`**, **`merged_evidence_len`**, **`section_drafts_count`**, **`writer_model`**.
2. **Evidence fingerprint:** e.g. hash or sorted URLs of the **`evidence`** list **`write_report`** actually uses — compare **after `prepare_writer_context`** vs **at `write_report` start** to detect subset loss without dumping full text.
3. **Full prompt archive on failure:** When eval fails or a debug flag is set, persist **full** `write_report` and per-section `write_sections` prompts to object storage or a large artifact (not the truncated process log).
4. **Deterministic post-check:** Parse **`[n]`** citations in **`report_markdown`** and verify each **n** exists in **`sources`** (and optionally that the URL appeared in curated evidence); emit **`citation_integrity`** into trace or metrics.
5. **Correlate logs and LangSmith:** Log **`thread_id`** / run id on each line when available so process logs and traces align.

**Interview shorthand:** Wrong report + good merge/prepare → **debug the writer path** (drafts vs assembly, conflicts block, citations vs `sources`, exact LLM inputs). Today: **truncated `--log`**, **`research_trace` counts**, **LangSmith**. Add: **writer metadata, evidence fingerprint, full prompts on failure, citation integrity checks.**
...
- **Retries / 429:** No automatic retries today; any exception from one batched query fails the whole node and stops the run (see [§6.7](#67-retries-timeouts-and-search-failure-eg-http-429)).
...
### 6.7 Retries, timeouts, and search failure (e.g. HTTP 429)

**Loop (for one section):** `generate_section_queries` → `section_search` → `section_normalize` → `section_assess_coverage` → **`section_route`** → either `generate_section_summary` (exit) or back to `generate_section_queries` (another iteration).

#### Where to put **timeouts**

| Layer | Rationale |
|-------|-----------|
| **HTTP / SDK calls inside search** | Timeouts belong on the **outbound request** (e.g. `httpx`/`requests` timeout, provider client default, or `urlopen(..., timeout=…)` where used). Cap **per query** and **per URL fetch** so one slow provider does not hang the whole worker. |
| **LLM calls** (`generate_section_queries`, `section_normalize`, `section_assess_coverage`, `generate_section_summary`) | Use the **chat model client’s** timeout / `request_timeout` (and optionally a **wrapper** `asyncio.wait_for` at the node if the client does not expose it). Keeps one stuck completion from blocking a section forever. |
| **Whole-node “watchdog”** | Optional **upper bound** per node invocation for ops (e.g. cancel and mark failed). Prefer **finer-grained** timeouts on I/O first so retries know whether the call actually completed. |

#### Where to put **retries**

| Layer | Rationale |
|-------|-----------|
| **Per search query (inside `_run_one_query_async` / provider helpers)** | **Best default:** Retry **one failed query** with **exponential backoff + jitter** on **429**, **503**, and **timeouts**, with a **max attempts** cap. Other queries in the same `asyncio.gather` batch can still succeed; you avoid failing the **entire** section on a single transient 429. |
| **Whole `section_search` node** | Retry only if the node is **idempotent** (same queries → same intended effect) and you **clear or merge** partial state carefully. Usually **worse** than per-query retry because the node already aggregates multiple queries. |
| **LLM nodes** | Same pattern as §25 **Retries**: retry **`ainvoke`** on **429/503** (and optionally **500**), not on most **4xx**; cap attempts. JSON parse failures are already handled with fallbacks in several nodes—retry the **LLM call** only when the failure is clearly transient. |
| **Rate limits (429)** | Retries **must** be paired with **backoff** and ideally **global throttling** (see [§25 — Rate-limit strategy](#25-error-handling-retries-fallbacks-and-rate-limit-strategy)); otherwise parallel workers amplify 429s. |

#### Failure model **today** when `section_search` hits a 429 (or any exception from one query)

**File:** `deep_research/nodes/section_search.py` — queries run with **`asyncio.gather(..., return_exceptions=True)`**. If **any** task returns an **`Exception`** (including a rate-limit error surfaced as an exception from the provider stack), the node **stops** and returns:

- **`section_raw_results`: `[]`**
- **`section_seen_urls`: `set()`**
- **`error_message`:** `"Search API failed: …"` (stringified exception)

**Subgraph routing** (`deep_research/section_graph.py`): after `section_search`, **`after_section_search_route`** sends to **`END`** if **`error_message`** is set—so the worker **does not** run `section_normalize`, `section_assess_coverage`, or `generate_section_summary` for that section.

**Parent graph** (`deep_research/graph.py`): **`after_section_worker_route`** sends the run to **`report_search_error`** (then **`__end__`**) when **`error_message`** is present—so **the entire Phase 2 run stops**, not just the failing section.

**Implications:** A **single** failed query in a batch fails **the whole section worker** and then **stops the main graph**. There is **no** in-code retry on 429 today; the failure is **fail-fast** at the first exception in the gather loop. For production, **per-query retry + backoff** (and concurrency limits) change this into **transient recovery**; without them, 429 is effectively a **hard stop** for the run.
...
- **conflict_route (Phase 2):** After `detect_global_gaps_and_conflicts`. Goes to `conflict_resolution_research` if resolution needed and enabled; else to **`eval_stop_gate`** (in `graph.py` the branch key is `prepare_writer_context`, but it targets the **`eval_stop_gate`** node).
- **stop_eval_route (Phase 2):** After **`eval_stop_gate`**. Goes to `conflict_resolution_research` if research is **not** sufficient and `research_retry_count <= 1`; else to `prepare_writer_context`.

All routing is **rule-based** (state fields and config), not LLM-decided, so the graph is predictable and debuggable. **`eval_stop_gate`** itself uses an LLM judge, but **graph transitions** after it are still rule-based (`stop_eval_route`).
...
| **Eval stop gate + `stop_eval_route`** | After merge/conflicts, an LLM judge on **`research_trace`** can trigger **one** bounded retry through conflict resolution before writing (see §5.10–5.11). |
...
8. **merge_section_evidence → detect_global_gaps_and_conflicts → conflict_route → [optional conflict_resolution_research] → eval_stop_gate → stop_eval_route → [optional second conflict_resolution_research] → prepare_writer_context → write_sections → write_report → finalize_messages → END.**  
   If **`error_message`** is set after **`section_worker`**, **`after_section_worker_route`** goes to **`report_search_error`** → END instead (see §6.7).

So on the **first invoke**, the very first step is **ingest_request**: it turns the single user message into a normalized `query` and a fully initialized (but empty) research state; every subsequent node then runs in order, with section workers fanning out after decompose and their results merged back before conflict detection, **eval stop gate**, and writing.
...
- **Branching:** after coverage → “continue research” vs “prepare writer”; after conflict detection → “resolve” vs “eval stop gate”; after **eval_stop_gate** → “retry conflict resolution” vs “prepare writer”; section worker → “another iteration” vs “summarize”.
...
| First non-empty wins | `_keep_first` | `query`, `section_max_iterations`, `error_message` (parallel workers) |
...
### Why not one reducer for everything?

Parallel `Send` branches each produce a partial state update; LangGraph **folds** those updates using the **reducer registered for each key**. The reducer must match the **meaning** of the field and whether you expect **one shared value** or **many contributions**.

- **Shared scalars (`query`, `section_max_iterations`):** Every worker receives the same values from the parent payload. Each branch may still emit those keys. You want **exactly one** authoritative string/int in merged state, not N copies combined—hence **`_keep_first`** (values should agree; the reducer only makes merge order well-defined).
- **Per-branch artifacts (`section_results`):** Each worker adds **one section’s** result. You need **all** of them—hence **`operator.add`** on lists (concatenation / extend semantics).
- **Sets (`seen_urls`, `global_seen_urls`, `section_seen_urls`):** Branches discover **different** URLs; you want the **union**, not “first wins” and not list concatenation of sets—hence **`_merge_sets`**.

**What if you used the same reducer for every key?**

- **`operator.add` everywhere:** For **strings**, `+` **concatenates**—N workers sending the same `query` could yield one repeated blob. For **ints** like `section_max_iterations`, add **sums** values and **inflates** the cap. For **`error_message`**, you’d get concatenated or scrambled messages. Wrong semantics for scalars.
- **`_keep_first` everywhere:** For **`section_results`**, you’d keep **only one worker’s list** and **drop the other sections’ evidence and summaries**—`merge_section_evidence` would run on a **partial** pipeline. Catastrophic for parallel research.
- **`_merge_sets` everywhere:** Only appropriate for **set[str]** fields; meaningless for lists of dicts or strings.

So reducers are not arbitrary: they encode **idempotent / shared fields** vs **additive per-worker outputs** vs **set union**. Mixing them up either **corrupts scalars** or **loses parallel work**.
...
Nodes are chosen so that: (1) each has one main job, (2) LLM vs I/O vs pure Python is clear, (3) routing and loop exits are attached to specific nodes (coverage, **conflict_route**, **stop_eval_route**), and (4) progress and traces stay interpretable. Combining some nodes is possible but would trade off that clarity.
...
**Why custom parser here:** Either the **schema is complex or variable** (plan, section_tasks, section summary), or we only need **simple JSON** (e.g. a list of strings) and a fallback is acceptable. Using `invoke` + strip-fences + `json.loads` keeps the prompt flexible and avoids defining a Pydantic model for every variant; the cost is possible malformed or non-JSON text, so we always use try/except and sensible defaults. **Operational risk:** generic fallbacks can **replace** the model’s plan or section list **without** an obvious error—see **§5.4 — JSON parsing fallbacks** (under `decompose_into_sections`) for concrete defaults, risks, and hardening.
...
- **Routing:** `route`, `section_route`, `conflict_route`, and `stop_eval_route` are tested with different state (sufficient/insufficient coverage, iteration cap, conflict on/off, resolution enabled/disabled, eval stop sufficient/insufficient).
...
Evals are run **after** report generation via **`async_run_evals`** (`deep_research/evals/__init__.py`) when **`run.py`** is invoked with **`--eval`** (default on; use **`--no-eval`** to skip). Scores are printed and also written to **`report_<query>_<timestamp>_evals.json`**; **`run.py` does not** change **exit code** based on scores—no deploy gate in code. See **§23 — Deploy gating** for how to use evals to block bad releases (e.g. **`citation_relevance`** regressions).
...
- **JSON file:** When **`--eval`** is on, `run.py` writes **`report_<query>_<timestamp>_evals.json`** next to the report (`score`, `reasoning`, and for some evals `section_scores`). Aggregate these in CI or a batch job for regression checks (§23).

- **Programmatically:** `run_evals(...)` / `async_run_evals(...)` return a dict `{eval_name: (score, reason)}` (or extended tuples where applicable). You can call them from tests or scripts without running the full graph.

- **Tests:** `tests/test_graph.py` defines `test_evals_run()`, which calls `run_evals(...)` with dummy report/evidence/trace and asserts that all 10 eval names are present and scores are in [0, 1].

- **Trace file:** The `--trace` flag writes `report_<query>_<timestamp>_trace.json` with `research_trace` only. **Eval scores** live in **`*_evals.json`** (with `--eval`) or stdout—not in the trace JSON.
...
- **Conditional edges:** Which branch was taken after `assess_coverage`, `section_route`, `conflict_route`, or `stop_eval_route`, and why (e.g. “budget exhausted” vs “section complete” vs “retry conflict resolution”). That explains loop behaviour and early exits.
...
For **bad final report text when merge and `prepare_writer_context` look fine**, see [§5.17](#517-debugging-wrong-final-report-when-merged_evidence-and-prepare_writer_context-look-correct) (writer path, what is logged today, what to add).
...
**Gaps:** No automated check that a citation’s *number* matches the right evidence item (e.g. [3] pointing to the wrong URL). DESIGN.md mentions a future “citation verification loop” (flag hallucinated citations, regenerate if below threshold). We don’t run that today. **Pipeline note:** `state["sources"]` is built from the evidence list passed into `write_report`, not parsed from the model’s markdown—so **spurious URLs in the report body** are a **writer-generation** problem; see **§5.15 — “Report cites a URL that never appeared in evidence”** for leak points and deterministic mitigations.
...
### Deploy gating (CI / releases): using evals to block a bad deploy

Evals are suited to catch **prompt or model regressions**—for example a writer change that tanks **`citation_relevance`** (attribution quality vs evidence snippets).

**Recommended pipeline**

1. **Golden dataset** — Versioned list of queries (and optionally pinned **config** / seeds). Same inputs every CI run; store in-repo JSON, LangSmith dataset, or DB.
2. **Batch in CI** — On each PR or release candidate: run the graph on **all** (or a stratified subset of) golden queries with **`--eval`** (and fixed **`--config`**). Collect every **`*_evals.json`** or call **`async_run_evals`** on saved report + evidence artifacts.
3. **Thresholds** — Per eval, define **blocking** rules: e.g. mean **`citation_relevance` ≥ 0.75** across the suite, or **no query** below **0.5** for **`claim_support`** / **`factual_accuracy`**. **Exit non-zero** from the CI script if any rule fails.
4. **Regression vs baseline** — Stricter: fail if mean **`citation_relevance`** (or another critical metric) **drops by more than Δ** vs **`main`** on the **same** dataset—catches relative regressions even when absolute scores look acceptable.
5. **Blocking subset** — Not every eval need block deploy; typical **gate** candidates: **`citation_relevance`**, **`claim_support`**, **`factual_accuracy`**. Keep **`tool_trajectory`** / **`comparative_breadth`** as **informational** unless product requires them.
6. **Artifact metadata** — Record **prompt/config hash**, **git SHA**, and **judge model** (`deep_research/evals/judge.py`) beside scores so regressions are attributable.
7. **Flakiness** — LLM-as-judge and writer variance: use **temperature 0**, optional **repeated runs + median**, or **tolerance bands** to avoid flaky red CI.

**Example failure:** A PR edits **`REPORT_ASSEMBLY_PROMPT`**; CI runs 20 golden queries; mean **`citation_relevance`** falls from **0.82** to **0.61** → **merge blocked** until the prompt is fixed or the threshold retuned with review.

### What’s missing today

- No curated **eval dataset** or versioned golden set in-repo for CI.
- No **threshold** or **exit-code gate** in `run.py`; `*_evals.json` is written, but nothing **fails the process** when scores are low.
- No **baseline comparison** (PR vs `main`) in code; you’d add a small aggregator script or workflow step.
- **CI cost / flakiness:** Full runs hit **live search + LLM** APIs unless you add **recorded fixtures** (cached evidence + report) or **mocked** search for eval-only replay.
- No **online sampling** or async eval pipeline in a hosted service; evals are **CLI / batch** oriented.
- **Optional:** Persist eval rows to a **warehouse** / LangSmith feedback for dashboards and alerts (beyond per-run JSON files).

So: **today** evals are **strong signals** and **persisted per run** as JSON, but **policy** (pass/fail, regression vs baseline) is **outside** the repo. A full **production pipeline** layers **deploy gating** (above) on top of the **offline / online** roles already described: batch over a fixed dataset, aggregation and regression checks, optional sampled async evals, and dashboards/alerts.
...
### Production runbook: LLM provider outage (e.g. OpenAI down for hours)

**How the CLI and graph behave today**

- **Critical path:** Nearly every node uses **`ChatOpenAI`** (or structured output on top of it) for classify, plan, decompose, section queries, normalize, coverage, summaries, conflicts, writer, eval judge, etc. Those calls follow the **LLM row** in the table above: **no** node-level try/except that returns a graceful state update for provider outages.
- **`run.py`:** The graph runs via **`graph.ainvoke`** / **`graph.astream`** inside **`try`/`finally`**; **`finally`** only closes the process log. There is **no** **`except`** around the graph that maps API failures to a user message. If any **`ainvoke`** raises (**503**, **5xx**, **connection errors**, timeouts during a broad outage), the exception **propagates**, **`async_main()`** terminates, and the process exits with a **Python traceback**—not a one-line “LLM unavailable” message.
- **Search vs LLM failure:** **`run.py`** treats **`state["error_message"]`** (set by **`section_search`** on search API failure) as a **clean** path: print message and **`sys.exit(1)`**. That path does **not** apply when **OpenAI** is down—search may still work.
- **Partial progress / resume:** Default CLI uses an **in-memory** checkpointer for interrupts; a **crash** does not automatically resume. **Evals** (`--eval`) run only after a **completed** successful graph.

**Operations runbook (owner in production)**

1. **Confirm** it is a **provider outage** (status page, dashboard) vs **quota** vs **your** deploy only—use a **minimal completion probe** if needed.
2. **Triage** — Page on-call; publish **user-visible status**; **pause** batch jobs and CI that call the LLM API to avoid noise and wasted quota.
3. **Degrade** — For an HTTP API product: return **503** with **`Retry-After`** (or **429** with clear body); **queue** new research jobs with backoff; turn off **non-essential** LLM features (e.g. evals, optional enrichment).
4. **Failover** — **Azure OpenAI**, **another region**, or **backup model** only if **pre-provisioned** in secrets and config (`OPENAI_BASE_URL`, model allowlist). This repo does **not** swap to Anthropic or local models **without code changes**.
5. **Recovery** — Drain the **queue** when healthy; **replay** dead-letter jobs; use a **persistent** checkpointer + hosted runner if you need **resume** after transient failures.

**What to add (engineering)**

| Layer | Addition |
|-------|----------|
| **LLM client** | **Retries** with exponential backoff + jitter on **429/503**; **timeouts**; **circuit breaker** (stop calling after repeated failures for a window). |
| **Nodes / graph** | Optional **try/except** around **`ainvoke`**: set **`error_message`** (or `llm_unavailable`) and route to **END** with copy safe for users—mirror the **search** failure pattern. |
| **CLI** | **`except`** around graph invocation: print a **single** error line + **`sys.exit(2)`** (distinct from search **`exit(1)`**). |
| **Config** | Documented **fallback model** / **Azure** endpoint; **feature flag** to disable research or run **cached-only** mode. |
| **Ops** | **Queue + DLQ**, **synthetic health** check for LLM reachability, **alerts** on error rate and latency **before** users report total failure. |

**Interview shorthand:** **OpenAI is on the critical path** with **no** first-class outage UX in **`run.py`**—expect **tracebacks** and **no report**. **Runbook:** detect → communicate → **stop harm** (pause consumers) → **queue** → failover if wired → **retries + circuit breaker + graceful `error_message` paths** for LLM failures (not only search).
...
**Current state:** No rate limiting. We send as many LLM and search requests as the graph needs. Under load, providers (OpenAI, Tavily, Exa, Gensee) may return **429**; we do not throttle or queue.

#### Why parallel sections multiply load

Concurrency is **not** just `max_parallel_sections` (from config `section.max_parallel` → `max_parallel_sections`). There are **two layers**:

1. **Graph layer:** `dispatch_sections` fans out up to **N** `section_worker` subgraphs at once (`deep_research/nodes/decompose.py`, `deep_research/graph.py`).
2. **Inside each worker:** `section_search` runs up to `section_queries_per_iteration` queries with **`asyncio.gather`** (`deep_research/nodes/section_search.py`). That can repeat each **`section_max_iterations`** loop.

So in the worst case, **concurrent search HTTP calls** scale roughly like **`max_parallel_sections × section_queries_per_iteration`** per “wave” of searches (before counting Phase 1 `run_search`, conflict-resolution search, writer-context fetches, etc.). **LLM** calls add a separate dimension: each section worker issues multiple model calls (queries, normalize, coverage, summary, …), all while other workers do the same—so **OpenAI RPM/TPM** limits matter independently of search.

**Design principle:** **Limit breadth** (how many sections run at once) and **limit depth** (how many in-flight requests share the network boundary), with **separate** caps for **search** vs **LLM** APIs.

#### What to rate limit (split by resource)

| Resource | Why separate |
|----------|----------------|
| **Search (Exa / Tavily / Gensee)** | Different vendors, different RPM and concurrency contracts. |
| **LLM (OpenAI, etc.)** | Token/rate limits are independent of search; section workers are LLM-heavy. |

#### How to add rate limiting (recommended patterns)

1. **Per-run caps (already partially there):** `max_iterations`, `section_max_iterations`, `max_parallel_sections`, and `section_queries_per_iteration` cap work and **indirectly** cap request volume. Lowering **`max_parallel_sections`** is the fastest operational lever when APIs throttle—no code change.

2. **Global concurrency cap at the search I/O boundary (strong first code step):** Add a **process-wide `asyncio.Semaphore(K)`** acquired around the code that performs each search HTTP request—ideally inside **`_run_one_query_async`** in `deep_research/nodes/search.py` (or immediately before each provider client call). All section workers and Phase 1 search then share one cap (e.g. **at most 4 concurrent search requests** process-wide), even when **`max_parallel_sections` is 10**. This addresses **multiplicative** fan-out (`sections × gather(queries)`).

3. **Token bucket / RPM per provider:** If vendors publish **requests per minute** (not just max connections), add a **token bucket** or sliding window **per provider** (Exa vs Tavily vs Gensee) so one saturated API does not block another. Libraries such as **`aiolimiter`** or a small Redis-backed limiter (multi-worker deployments) work here.

4. **LLM concurrency / RPM:** Add a **second semaphore** (or RPM limiter) around **`ainvoke` / `invoke`** paths, or rely on client **`max_retries`** plus a **global cap on concurrent LLM calls** if you hit TPM throttling across parallel sections.

5. **Backoff on 429 / 503:** Combine limiting with **retries** (see **Retries** above): exponential backoff + jitter, max attempts, respect **`Retry-After`** when present. Apply retries **inside** the limited critical section (after acquiring the semaphore) so retries do not amplify burst traffic.

6. **Config knobs (examples):** e.g. `search_max_concurrent_requests`, optional per-provider caps (`search_max_concurrent_exa`, …), `llm_max_concurrent_requests` or RPM settings. Defaults should be **conservative**: high **`max_parallel_sections`** must not imply unconstrained outbound HTTP.

7. **LangSmith:** LangSmith does not rate-limit your graph; it only receives trace data. Rate limiting is **between your process and the LLM/search APIs**.

8. **Multi-process / multi-replica:** An in-process semaphore does **not** coordinate across workers. For several processes or pods, use a **shared** limiter (Redis, DB lease, or API-gateway quotas) so cluster-wide concurrency stays under contract.

**Interview shorthand:** Parallelism is **multiplicative** (sections × queries per iteration × iterations). Cap **breadth** with config and **depth** with **shared semaphores or token buckets** at the **network boundary**, add **429-aware retries**, and treat **search limits and LLM limits separately**.

**Summary:** Today we have **localized** error handling and fallbacks (search → empty list, JSON → defaults, extraction → next method, eval → 0.5). We do **not** have retries, no runtime search-provider fallback, and **no** rate limiting. For production, add retries with backoff for LLM and search, optional provider fallback, and a rate-limit strategy: **per-run caps + global search concurrency (semaphore at `_run_one_query_async`) + optional per-provider RPM + separate LLM throttling + backoff on 429**.
